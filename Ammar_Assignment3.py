...
Pyramid1 = Pyramid(150, 100, 0) # creating new pyramid
Pyramid2 = Pyramid(-50, 150, 50)
Cube1 = Cube(100, -50, 0) # creating new cube
Cube2 = Cube(-100, -75, -50)
...
def fill(poly): # function that defines the fill algorithm
    ...
    if len(poly) == 3: # perform fill for any poly with 3 points
        ...
        for i in range(int(Ymin), int(Ymax)): # for loop to draw each line
            if za1b1 < q:
                w.create_line(edge1x, i, edge2x, i, fill = "blue") # create line between points
            else:
                pass
        ...
        if P1[1] < P2[1] and P1[1] < P3[1] and P1[1] < P4[1]: # find smallest Y in vertex
            ...
            edge4x = edge4[0][0]# assign edges
            
            for i in range(int(Ymin), int(Ymax)):
                w.create_line(edge2x, i, edge4x, i, fill = "blue")
                edge1x = edge1x + InvSlope1
                edge2x = edge2x + InvSlope2# updates edges
                edge3x = edge3x + InvSlope3 # udates edges
                edge4x = edge4x + InvSlope4
              
                if i > edge2[1][1]:
                    edge2x = edge3[0][0] #increments edges
                if i > edge4[1][1]:
                    edge4x = edge1[0][0] #increments edges
            ...
            InvSlope1 = (edge1[1][0] - edge1[0][0])/(edge1[1][1] - edge1[0][1]) # inv slope formula
            InvSlope2 = (edge2[1][0] - edge2[0][0])/(edge2[1][1] - edge2[0][1])     
            InvSlope3 = (edge3[1][0] - edge3[0][0])/(edge3[1][1] - edge3[0][1])# inv slope formula
            InvSlope4 = (edge4[1][0] - edge4[0][0])/(edge4[1][1] - edge4[0][1])
            ...
            for i in range(int(Ymin), int(Ymax)):
                w.create_line(edge2x, i, edge4x, i, fill = "blue")
                edge1x = edge1x + InvSlope1
                edge2x = edge2x + InvSlope2 # update edges
                edge3x = edge3x + InvSlope3
                edge4x = edge4x + InvSlope4# update edges
            ...
            if P1[1] < P2[1] and P1[1] < P4[1]: # if to find the min/mid/max
                ...
                if P1[1] < P2[1]: # if to find the min/mid/max
                    ...
                    edge4 = (P4, P1)# assigngs point to edge
            InvSlope1 = (edge1[1][0] - edge1[0][0])/(edge1[1][1] - edge1[0][1]) # inv slope calculator
            InvSlope2 = (edge2[1][0] - edge2[0][0])/(edge2[1][1] - edge2[0][1]) # inv slope calculator     
            InvSlope3 = (edge3[1][0] - edge3[0][0])/(edge3[1][1] - edge3[0][1]) # inv slope calculator
            InvSlope4 = (edge4[1][0] - edge4[0][0])/(edge4[1][1] - edge4[0][1]) # inv slope calculator
            ...
            for i in range(int(Ymin), int(Ymax)): # for loop to fill poly
                w.create_line(edge2x, i, edge4x, i, fill = "blue") # 
                edge1x = edge1x + InvSlope1
                edge2x = edge2x + InvSlope2 # update edges
                edge3x = edge3x + InvSlope3
            ...
            for i in range(int(Ymin), int(Ymax)): # creates for loop to fill poly
                w.create_line(edge2x, i, edge4x, i, fill = "blue") # fills line between points
                edge1x = edge1x + InvSlope1 # updates the edge
                edge2x = edge2x + InvSlope2# updates the edge
                edge3x = edge3x + InvSlope3 # updates the edge
                edge4x = edge4x + InvSlope4 # updates the edge
...
def drawObject(object):
    if curr_object.Base == object:
        ...
        drawPoly(object[1], 'red') # draws frontpoly by calling drawPoly
        drawPoly(object[2], 'red') # draws rightpoly by calling drawPoly
        drawPoly(object[3], 'red') # draws backpoly by calling drawPoly
        drawPoly(object[4], 'red') # draws leftpoly by calling drawPoly
        drawPoly(object[0], 'red') # draws bottompoly by calling drawPoly
    else:
